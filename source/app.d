...
int main(string[] args) {
	...
	try {
		auto opt = getopt(args,
			...
			"p|plotsize", "Set the size of the plot in blocks (Basic plots are 51, Large plots are 101, and Massive plots are 301). The compiler will generate warnings if any declaration is longer than the given size.", &flags.plotSize
		);
		if(opt.helpWanted) {
			writeln("Embyr Compiler V"~VERSION);
			writeln("Usage: ", args[0], " [options] <input files...>");
...
6 - Argument Error`);
	...
	foreach(i; inputs) {
		try {
			...
			decls ~= compile(i, text, ast, flags.addtags);
	...
	if(flags.plotSize != 0) {
		foreach(d; decls)
			if(d.size > flags.plotSize) 
				writeln("Warning: Size of declaration '", d.name, "' is ", d.size, ", which is greater than specified plot size ", flags.plotSize, ".");
	}
